#!/usr/bin/env python3
# Stable spatial experiment runner (v2 schema) using v1-style warmup + SG init
...
from spatial_experiment.planners.vlm_planner_gemini_spatial_v2 import (
    VLMPlannerEQAGeminiSpatialV2,
)
...
# -------- SG update guard: skip when imgs list is empty (prevents crashes) --------
class _SGUpdateGuard:
    ...
    def update(self, *args, **kwargs):
        ...
        if isinstance(imgs, _np.ndarray):
            ...
            frames = [im for im in imgs if im is not None]
        if imgs is not None and len(frames) == 0:
            print("[V2 Guard] sg_sim.update skipped: no RGB frames this step.")
            return
        return self._base.update(*args, **kwargs)


# -------- Warm-up & SG init: same shape as the known-good v1 runner --------
def _warmup_like_eqa(habitat_data: HabitatInterface, pipeline, out_dir, rr_logger, tsdf_planner, sg_sim):
    """Use the same warmup pattern as v1 runner."""
    ...
    try:
        run(
            ...
            rr_logger=rr_logger, tsdf_planner=tsdf_planner, sg_sim=_SGUpdateGuard(sg_sim),
...
def _ensure_scenegraph_initialized(habitat_data: HabitatInterface, pipeline, out_dir, rr_logger, tsdf_planner, sg_sim):
    """If SG didn’t init during warmup, force one more tiny tick using the same pose API."""
    needs_init = (
    ...
    try:
        run(
            ...
            tsdf_planner=tsdf_planner, sg_sim=_SGUpdateGuard(sg_sim),
            save_image=True, segmenter=None,
        )
    except Exception as e:
...
def _teleport_agent(habitat_data: HabitatInterface, init_pos, init_rot):
    ...
    state.position = np.asarray(init_pos, dtype=np.float32)
    state.rotation = quat_from_coeffs(np.asarray(init_rot, dtype=np.float32))
...
def main(cfg):
    ...
    for task_index, task_data in enumerate(tqdm(tasks_data, desc="Running Tasks (v2)")):
        ...
        pipeline = None
        try:
            click.secho(f"[v2] Executing task: {experiment_id}", fg="green")
            ...
            # ---------- Spawn: dataset initial_pose (trusted from JSON) ----------
            ...
            click.secho("[spawn] dataset initial_pose (trusted)", fg="cyan")

            # ---------- TSDF (kept to satisfy run(); harmless with RRLogger) ----------
            ...
            sg_sim = SceneGraphSim(
                ...
                device=device, clean_ques_ans=question, enrich_object_labels=" ",
            )

            # ---------- Warm-up like v1 (reliable SG bootstrap) ----------
            ...
            # Re-teleport to the dataset spawn before planning (warmup moves agent)
            _teleport_agent(habitat_data, init_pos, init_rot)
            ...
            # ---------- Planner (v2) ----------
            vlm_planner = VLMPlannerEQAGeminiSpatialV2(
            ...
            for step_count in range(num_steps):
                ...
                # Convert normal-frame target to habitat frame
                ...
                try:
                    ...
                    pass  # keep running even if logger complains
                ...
                run(
                    ...
                    rr_logger=rr_logger, tsdf_planner=tsdf_planner,
                    sg_sim=_SGUpdateGuard(sg_sim), save_image=cfg.vlm.use_image, segmenter=None,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="spatial_vqa_v2", type=str, required=False,
    )
