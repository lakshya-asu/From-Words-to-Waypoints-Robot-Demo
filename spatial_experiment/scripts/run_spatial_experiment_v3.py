#!/usr/bin/env python3
# /home/artemis/project/graph_eqa_swagat/spatial_experiment/scripts/run_spatial_experiment_v3.py
"""
v3 runner:
- Uses v3 dataset JSON (with candidate size [w,d,h] included).
- Boots TSDF + SceneGraph as before for stable SG.
- Plugs in VLMPlannerEQAGeminiSpatialV3 (MSP bridge).
"""
...
from spatial_experiment.planners.vlm_planner_gemini_spatial_v3 import (
    VLMPlannerEQAGeminiSpatialV3,
)
...
def main(cfg):
    ...
    for task_index, task_data in enumerate(tqdm(tasks_data, desc="Running Tasks (v3)")):
        ...
        pipeline = None
        try:
            click.secho(f"[v3] Executing task: {experiment_id}", fg="green")
            ...
            # Re-teleport back to dataset pose (warmup moved agent)
            _teleport_agent(habitat_data, init_pos, init_rot)
            ...
            # Planner V3 (MSP-augmented)
            vlm_planner = VLMPlannerEQAGeminiSpatialV3(
            ...
            gt_id = gt["id"]
            # success: exact GT id or (optionally) any dataset candidate
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="spatial_vqa_v3", type=str, required=False,
    )
