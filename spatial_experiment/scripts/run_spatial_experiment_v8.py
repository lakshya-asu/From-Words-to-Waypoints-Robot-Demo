...
- Passes 'agent_yaw_rad' and 'agent_pos_hab' to the planner for Affordance-Aware reasoning.
- Fixes SceneGraphSim initialization to prevent 'NoneType' errors.
- Adds detailed logging so we can see how theta0 is derived each step.
"""
...
# --- V7 IMPORT ---
from spatial_experiment.planners.vlm_planner_gemini_spatial_v8 import (
    VLMPlannerEQAGeminiSpatialV8,
)
...
def main(cfg):
    ...
    for task_index, task_data in enumerate(tqdm(tasks_data, desc="Running Tasks (V7 Gemini)")):
        ...
        try:
            ...
            vlm_planner = VLMPlannerEQAGeminiSpatialV8(
            ...
            for step_count in range(num_steps):
                # Get current agent pose and heading for this step
                agent = habitat_data._sim.get_agent(0)
                agent_state = agent.get_state()
                current_pos_hab = np.array(agent_state.position, dtype=np.float32)
                current_heading = float(habitat_data.get_heading_angle())

                click.secho(
                    f"[step {step_count}] Agent pos={current_pos_hab}, yaw={current_heading:.3f} rad",
                    fg="magenta",
                )
                
                target_pose, target_id, is_confident, confidence_level, target_declaration = (
                    vlm_planner.get_next_action(
                        agent_yaw_rad=current_heading,
                        agent_pos_hab=current_pos_hab,
                    )
                )
                ...
                # Refresh agent state in case anything changed
                agent = habitat_data._sim.get_agent(0)
                ...
                poses = habitat_data.get_trajectory_from_path_habitat_frame(
                    target_pose, desired_path_norm,
                    habitat_data.get_heading_angle(), cfg.habitat.camera_tilt_deg
                )
                if poses is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="spatial_vqa_v8", type=str, required=False,
    )
